...
function swap(oldText, newText, label) {
  ...
  if (count !== 1) {
    throw new Error(`TornPulse v0.8.0 patch: expected exactly 1 match for ${label}, found ${count}`);
  }
...
function swapAll(oldText, newText, label) {
  ...
  if (count === 0) {
    ...
    throw new Error(`TornPulse v0.8.0 patch: no matches for ${label}`);
  }
...
// -------- Version / branding --------
swap("config.version = '0.7.3';", "config.version = '0.8.0';", 'Expo version');
swap("versionCode: 18,", "versionCode: 19,", 'Android versionCode');
...
swapAll("0.7.2", "0.8.0", 'visible app version');
swapAll("COMPACT HUD", "NOIR HUD", 'visible build label');
